...
class Robolink:
    ...
    def AddCurve(self, curve_points, reference_object=0, add_to_ref=False,
                 projection_type=PROJECTION_ALONG_NORMAL_RECALC):
    ...
    def BuildMechanism(self, type, list_obj, parameters, joints_build, joints_home, joints_senses, joints_lim_low,
                       joints_lim_high, base=eye(4), tool=eye(4), name="New robot", robot=None):
    ...
    def setInteractiveMode(self, mode_type=SELECT_MOVE, default_ref_flags=DISPLAY_REF_DEFAULT, custom_objects=None,
                           custom_ref_flags=None):
    ...
    def EmbedWindow(self, docked_name, window_name, size_w=-1, size_h=-1, pid=0, area_add=1, area_allowed=15,
                    timeout=100):
...
class Item():
    ...
    def MoveJ(self, target, blocking=True):
        ...
        if self.type == ITEM_TYPE_PROGRAM:
            if type(target) != Item:
                raise Exception(
                    "Adding a movement instruction to a program given joints or a pose is not supported. Use a target item instead, for example, add a target as with RDK.AddTarget(...) and set the pose or joints.")
